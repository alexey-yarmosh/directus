...
0.3. remove `--ignore extensions` from api/package.json if extesnion is not auto reloaded

1. npm run init:dev
6. Run `pnpm --filter api dev` to view the logs. Verify that there are no errors.
7. Run only VSCode debug, that is enough to run both BE and FE. Debugging doesnt work with symlink so you need to rm the extension symlink and copy the folder. (you can try to add --ignore extensions in package.json if it reloads too long)
